#!/usr/bin/env python3
"""
Method 1: Boruta Feature Selection with Complete ML Pipeline
Implements wrapper-based feature selection using Boruta algorithm
"""
...
def create_comprehensive_analysis(overall_results, output_path, stage_names):
    ...
    for stage in stage_names:
        ...
        mape_data = mape_data[mape_data < 100]  # Filter extreme values
    ...
    plt.suptitle('Comprehensive Model Performance Analysis - Boruta Method', 
...
def create_lr_comparison(overall_results, output_path):
    ...
    metrics_norm['R2_Test'] = metrics_norm['R2_Test']  # Keep R² as is
    metrics_norm['RMSE'] = metrics_norm['RMSE'] / metrics_norm['RMSE'].max()  # Normalize
    metrics_norm['MAE'] = metrics_norm['MAE'] / metrics_norm['MAE'].max()  # Normalize
    ...
    ax4 = axes[1, 1]
    error_pivot = lr_data.pivot_table(values=['RMSE', 'MAE', 'MAPE'], 
                                      index='Stage', columns='Split', aggfunc='mean')
    
    # Create a simplified heatmap with RMSE only
    ...
    plt.suptitle('Linear Regression Performance Analysis - Boruta Method', 
...
def perform_boruta_selection(X, y, feature_names):
    ...
    try:
        ...
        print(f"\n✓ Boruta completed!")
        print(f"  Selected features: {sum(selected_mask)}/{len(feature_names)}")
        print(f"  Selected: {', '.join(selected_features)}")
        
        return selected_features, results_df, boruta_selector
        
    except Exception as e:
        print(f"✗ Boruta failed: {str(e)}")
        ...
        return feature_names, results_df, None

def visualize_boruta_results(results_df, output_path):
    """Create visualization of Boruta results"""
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    
    # 1. Feature rankings bar chart
    ax1 = axes[0]
    colors = ['green' if s else 'red' for s in results_df['Selected']]
    ax1.barh(results_df['Feature'], results_df['Ranking'], color=colors, alpha=0.7, edgecolor='black')
    ax1.set_xlabel('Ranking (Lower is Better)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Features', fontsize=12, fontweight='bold')
    ax1.set_title('Boruta Feature Rankings', fontsize=14, fontweight='bold')
    ax1.invert_yaxis()
    ...
    # 2. Selection summary pie chart
    ax2 = axes[1]
    selection_counts = results_df['Decision'].value_counts()
    colors_pie = ['#2ecc71' if 'Selected' in label else '#e74c3c' for label in selection_counts.index]
    wedges, texts, autotexts = ax2.pie(selection_counts.values, labels=selection_counts.index, 
                                        autopct='%1.1f%%', colors=colors_pie, startangle=90,
                                        textprops={'fontsize': 11, 'fontweight': 'bold'})
    ax2.set_title('Boruta Selection Summary', fontsize=14, fontweight='bold')
    
    plt.suptitle('Boruta Feature Selection Results', fontsize=16, fontweight='bold', y=1.00)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✓ Boruta visualization saved: {output_path}")
...
def main():
    print("\n" + "="*70)
    print("BORUTA FEATURE SELECTION & ML PIPELINE")
    print("="*70)
    ...
    try:
        ...
        base_path = os.path.join(output_base, "Method1_Boruta")
        stages = ['Before_Boruta', 'After_Boruta']
        create_directory_structure(base_path, stages)
        
        # Perform Boruta selection
        selected_features, boruta_results, boruta_selector = perform_boruta_selection(X, y, feature_names)
        
        # Save Boruta results
        boruta_report_path = os.path.join(base_path, "Boruta_Selection_Report.csv")
        boruta_results.to_csv(boruta_report_path, index=False)
        print(f"✓ Boruta report saved: {boruta_report_path}")
        
        # Visualize Boruta results
        boruta_viz_path = os.path.join(base_path, "Boruta_Rankings.png")
        visualize_boruta_results(boruta_results, boruta_viz_path)
        ...
        # Process each stage
        for stage_idx, (stage_name, features_to_use) in enumerate([
            ('Before_Boruta', feature_names),
            ('After_Boruta', selected_features)
        ]):
            print("\n" + "="*70)
