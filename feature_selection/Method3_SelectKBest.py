#!/usr/bin/env python3
"""
Method 3: SelectKBest with F-test Feature Selection & Complete ML Pipeline
Implements filter-based feature selection using F-test statistical method
"""
...
def create_comprehensive_analysis(overall_results, output_path, stage_names):
    ...
    ax7.set_xticklabels(best_data.index, rotation=45, ha='right', fontsize=8)
    ...
    plt.suptitle('Comprehensive Model Performance Analysis - SelectKBest Method', 
...
def create_lr_comparison(overall_results, output_path):
    ...
    plt.suptitle('Linear Regression Performance Analysis - SelectKBest Method', 
...
def perform_selectkbest_selection(X, y, feature_names):
    """Perform SelectKBest feature selection with multiple k values"""
    print("\n" + "="*70)
    print("SELECTKBEST (F-TEST) FEATURE SELECTION")
    print("="*70)
    
    try:
        n_features = len(feature_names)
        
        # Different k values: select top 80%, 60%, 40% of features
        k_values = {
            'SelectKBest_Top80%': max(1, int(n_features * 0.8)),
            'SelectKBest_Top60%': max(1, int(n_features * 0.6)),
            'SelectKBest_Top40%': max(1, int(n_features * 0.4))
        }
        
        all_results = {}
        
        for stage_name, k in k_values.items():
            print(f"\nRunning SelectKBest for {stage_name} (k={k})...")
            
            # Fit SelectKBest with F-test
            ...
            print(f"  Top 3 features by F-score: {', '.join(results_df.head(3)['Feature'].values)}")
        
        return all_results
        
    except Exception as e:
        print(f"✗ SelectKBest failed: {str(e)}")
...
def visualize_selectkbest_results(all_results, output_path):
    """Create visualization of SelectKBest results"""
    n_stages = len(all_results)
    fig, axes = plt.subplots(2, n_stages, figsize=(8*n_stages, 10))
    ...
    for idx, (stage_name, result_data) in enumerate(all_results.items()):
        ...
        # Top subplot: F-scores
        ax_top = axes[0, idx]
        colors = ['green' if s else 'red' for s in results_df['Selected']]
        ax_top.barh(results_df['Feature'], results_df['F_Score'], color=colors, alpha=0.7, edgecolor='black')
        ax_top.set_xlabel('F-Score', fontsize=11, fontweight='bold')
        ax_top.set_ylabel('Features', fontsize=11, fontweight='bold')
        ax_top.set_title(f'{stage_name}\nF-Scores (Higher is Better)', fontsize=12, fontweight='bold')
        ax_top.invert_yaxis()
        ax_top.grid(True, alpha=0.3, axis='x')
        
        # Bottom subplot: P-values (log scale)
        ax_bottom = axes[1, idx]
        p_values_log = -np.log10(results_df['P_Value'] + 1e-10)  # Add small value to avoid log(0)
        ax_bottom.barh(results_df['Feature'], p_values_log, color=colors, alpha=0.7, edgecolor='black')
        ax_bottom.set_xlabel('-log10(P-Value)', fontsize=11, fontweight='bold')
        ax_bottom.set_ylabel('Features', fontsize=11, fontweight='bold')
        ax_bottom.set_title('Significance (Higher is Better)', fontsize=12, fontweight='bold')
        ax_bottom.axvline(x=-np.log10(0.05), color='red', linestyle='--', label='p=0.05', alpha=0.7)
    ...
    plt.suptitle('SelectKBest (F-test) Feature Selection Results', fontsize=16, fontweight='bold', y=0.995)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✓ SelectKBest visualization saved: {output_path}")
...
def main():
    print("\n" + "="*70)
    print("SELECTKBEST (F-TEST) FEATURE SELECTION & ML PIPELINE")
    print("="*70)
    ...
    try:
        ...
        # Perform SelectKBest selection
        selectkbest_results = perform_selectkbest_selection(X, y, feature_names)
        
        # Create directory structure with all stages
        base_path = os.path.join(output_base, "Method3_SelectKBest")
        stages = ['Before_SelectKBest'] + list(selectkbest_results.keys())
        create_directory_structure(base_path, stages)
        
        # Save SelectKBest results
        for stage_name, result_data in selectkbest_results.items():
        ...
        selectkbest_viz_path = os.path.join(base_path, "SelectKBest_Rankings.png")
        visualize_selectkbest_results(selectkbest_results, selectkbest_viz_path)
        ...
        # Process each stage (Before + SelectKBest stages)
        stage_configs = [('Before_SelectKBest', feature_names)]
        for stage_name, result_data in selectkbest_results.items():
